...
function FormCats({ refresh }: IProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    axios
      .post("http://localhost:5000/api/v1/cats", addCard)
      .then((res) => {
  ...
  const handleImageUpload = (catId: string) => {
    ...
    axios
      .post(
        `http://localhost:5000/api/v1/cats/${catId}/upload/image`,
        formData,
        {
          headers: {
